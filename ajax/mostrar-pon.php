<?php   
     ...
     if (strlen($nom) >= 3) {  
          if ($_SESSION['wrktipusu'] >= 4) {
               $com = "Select C.idconta, U.usunome, P.prodescricao, P.protipo from ((tb_conta C left join tb_usuario U on C.conusuario = U.idsenha) left join tb_programa P on C.conprograma = P.idprograma) where constatus = 0 and protipo = 1 and conempresa = " . $_SESSION['wrkcodemp'] . " and usunome like '%" . $nom . "%'  order by usunome, prodescricao, idconta Limit 25"; 
          } else {
